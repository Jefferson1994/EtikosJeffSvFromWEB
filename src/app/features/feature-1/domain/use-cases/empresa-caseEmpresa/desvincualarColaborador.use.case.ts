...
export class desvincularColaboradorEmpresasUseCase {
  ...
  async execute(idEmpresa: number, id_usuario: number ): Promise<RespuestaDesvincular> {
    ...
    try {
      ...
      const repuesta = await this.repository.desvincularColaboradoresXEmpresa(idEmpresa,id_usuario);
      console.log("la desvinculacion:", JSON.stringify(repuesta));
      return repuesta;
    } catch (error) {
