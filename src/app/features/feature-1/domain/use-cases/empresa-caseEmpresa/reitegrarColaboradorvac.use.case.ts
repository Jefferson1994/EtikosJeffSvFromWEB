...
export class ReintegrarvacacacionesColaboradorEmpresasUseCase {
  ...
  async execute(idEmpresa: number, id_usuario: number ): Promise<RespuestaDesvincular> {
    ...
    try {
      ...
      const repuesta = await this.repository.IntegrarvacacionesColaboradoresXEmpresa(idEmpresa,id_usuario);
